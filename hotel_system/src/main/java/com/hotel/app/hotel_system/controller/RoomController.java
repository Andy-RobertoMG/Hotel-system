...
public class RoomController {
   ...
   public List<RoomDTO> FindAll(){
      ...
      for(Room habitacion: listaRooms){
         RoomDTO aux = new RoomDTO();
         aux.setFloor(habitacion.getFloor());
         aux.setId(habitacion.getId());
         aux.setRoomtype_id(habitacion.getRoomType2().getId());
         listaDTO.add(aux);
      }
      return listaDTO;
   }
   ...
   @PostMapping("/rooms")
   public ResponseEntity<Message> create(@RequestBody RoomDTO room) throws RoomTypeNotFoundException{
   ...
   public RoomDTO SearchById(@PathVariable Long id) throws RoomNotFoundException
   {
      ...
      RoomDTO encontrado = new RoomDTO();
      encontrado.setFloor(aux.getFloor());
      encontrado.setId(aux.getId());
      encontrado.setRoomtype_id(aux.getRoomType2().getId());
      return encontrado;
   }
}
